...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const uRot = new Float32Array([1, 0, 0, 0, 1, 0, 0, 0, 1])
    ...
    const pointer: { x: number; y: number; inside: boolean } = { x: 0, y: 0, inside: true }
    ...
    const render = (t: number) => {
      ...
      if (animationType === 'hover') {
        ...
        targetYaw = pointer.inside ? -pointer.x * maxYaw : 0
        targetPitch = pointer.inside ? pointer.y * maxPitch : 0
      ...
      if (continueRAF) raf = window.requestAnimationFrame(render)
      else raf = 0
    }
